(function ($) {
    ...
    if ($.fn.DataTable.isDataTable('#groupAnalytics-bootstrap-data-table')) {
        // DataTable is already initialized, destroy the existing instance
        $('#groupAnalytics-bootstrap-data-table').DataTable().destroy();
    }

    // Initialize DataTable
    dataTable = $('#groupAnalytics-bootstrap-data-table').DataTable({
    ...
    function addRow(title, status, deadline, workload, taskId) {
        ...
        const rowData = [
            ...
            formattedDeadline,
            workload,
            `<button type="button" style="background-color: #93BAE9; color: #1b3550;" class="btn  task-btn" data-toggle="modal" data-target="#taskGroupModal" data-task-id="${taskId}">
            ...
            </button>`
        ];
        ...
        button.addEventListener('click', function () {
            ...
            $.ajax({
                ...
                success: function (taskDetails) {
                    ...
                    const modalBody = document.querySelector('#taskGroupModalBody');
                    modalBody.innerHTML = `
